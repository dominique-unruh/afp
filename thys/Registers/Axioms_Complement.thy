...
axiomatization where 
  complement_exists_simple: \<open>reference F \<Longrightarrow> \<exists>G :: ('a, 'b) complement_domain_simple update \<Rightarrow> 'b update. disjoint F G \<and> iso_reference (F;G)\<close>
...
axiomatization cdc :: \<open>('a::domain update \<Rightarrow> 'b::domain update) \<Rightarrow> ('a,'b) complement_domain set\<close> where 
  complement_exists: \<open>reference F \<Longrightarrow> \<forall>\<^sub>\<tau> 'c::domain = cdc F.
...
axiomatization where complement_unique: \<open>disjoint F G \<Longrightarrow> iso_reference (F;G) \<Longrightarrow> disjoint F H \<Longrightarrow> iso_reference (F;H)
          \<Longrightarrow> equivalent_references G H\<close> 
