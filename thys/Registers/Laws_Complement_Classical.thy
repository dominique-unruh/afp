...
theory Laws_Complement_Classical
  imports Laws_Classical Axioms_Complement_Classical
begin
...
proof -
  ...
  have \<open>let 'x::type = complement_domain X in equivalent_registers X (U; X)\<close>
  proof with_type_mp
...
proof -
  ...
  have \<open>let 'x::type = complement_domain A in is_unit_register (A o U)\<close>
  proof with_type_mp
    note [[simproc del: compatibility_warn]]
    with_type_case 
...
proof -
  have \<open>is_unit_register ((\<lambda>d. tensor_update Some d) o G)\<close>
    by (simp add: assms(2) unit_register_compose_left)
  moreover have \<open>is_unit_register ((\<lambda>c. tensor_update c Some) o F)\<close>
    using assms(1) register_tensor_left unit_register_compose_left by blast
  ultimately have \<open>equivalent_registers ((\<lambda>d. tensor_update Some d) o G) ((\<lambda>c. tensor_update c Some) o F)\<close>
...
proof -
  ...
  have \<open>let 'x::type = complement_domain (id::'a update \<Rightarrow> _) in is_unit_register ?U\<close>
  proof with_type_mp
    with_type_case
...
lemma unit_register_domain_tensor_unit:
  ...
  (* Can we show that I = (\<lambda>x. tensor_update Some x) ? It would be nice but I do not see how to prove it. *)
proof -
  from complement_exists[OF register_id[where 'a='b]]
  have \<open>let 'x::type = complement_domain (id :: 'b update \<Rightarrow> _) in
