...
section \<open>Generic laws about references, instantiated quantumly\<close>
...
lemma prereference_tensor_left[simp]: \<open>prereference (\<lambda>b::'b::type update. tensor_op a b)\<close>
  for a :: \<open>'a::type update\<close>
proof -
  have \<open>prereference ((\<lambda>b1::('a\<times>'b) update. (a \<otimes>\<^sub>u id_cblinfun) *\<^sub>u b1) o (\<lambda>b. tensor_op id_cblinfun b))\<close>
    by (rule comp_prereference; simp)
  then show ?thesis
    by (simp add: o_def comp_tensor_op)
qed

lemma prereference_tensor_right[simp]: \<open>prereference (\<lambda>a::'a::type update. tensor_op a b)\<close>  
  for b :: \<open>'b::type update\<close>
proof -
  have \<open>prereference ((\<lambda>a1::('a\<times>'b) update. (id_cblinfun \<otimes>\<^sub>u b) *\<^sub>u a1) o (\<lambda>a. tensor_op a id_cblinfun))\<close>
...
lemma id_update_tensor_reference[simp]:
  ...
  shows \<open>reference (\<lambda>a::'a::type update. id_cblinfun \<otimes>\<^sub>u F a)\<close>
  using assms apply (rule reference_comp[unfolded o_def])
  by simp

lemma reference_tensor_id_update[simp]:
  assumes \<open>reference F\<close>
  shows \<open>reference (\<lambda>a::'a::type update. F a \<otimes>\<^sub>u id_cblinfun)\<close>
  using assms apply (rule reference_comp[unfolded o_def])
  by simp
...
definition reference_tensor  (infixr "\<otimes>\<^sub>r" 70) where
  "reference_tensor F G = reference_pair (\<lambda>a. tensor_op (F a) id_cblinfun) (\<lambda>b. tensor_op id_cblinfun (G b))"
...
definition "separating (_::'b::type itself) A \<longleftrightarrow> 
  (\<forall>F G :: 'a::type update \<Rightarrow> 'b update. prereference F \<longrightarrow> prereference G \<longrightarrow> (\<forall>x\<in>A. F x = G x) \<longrightarrow> F = G)"
...
lemma reference_eqI: \<open>separating TYPE('b::type) A \<Longrightarrow> prereference F \<Longrightarrow> prereference G \<Longrightarrow> (\<And>x. x\<in>A \<Longrightarrow> F x = G x) \<Longrightarrow> F = (G::_ \<Rightarrow> 'b update)\<close>
  unfolding separating_def by auto
...
definition disjoint :: \<open>('a::type update \<Rightarrow> 'c::type update)
...
lemma disjoint_tensor_id_update_left[simp]:
  ...
  assumes "disjoint F G"
  shows "disjoint (\<lambda>a. id_cblinfun \<otimes>\<^sub>u F a) (\<lambda>a. id_cblinfun \<otimes>\<^sub>u G a)"
...
lemma disjoint_tensor_id_update_right[simp]:
  fixes F :: "'a::type update \<Rightarrow> 'c::type update" and G :: "'b::type update \<Rightarrow> 'c::type update"
  assumes "disjoint F G"
  shows "disjoint (\<lambda>a. F a \<otimes>\<^sub>u id_cblinfun) (\<lambda>a. G a \<otimes>\<^sub>u id_cblinfun)"
  using assms apply (rule disjoint_comp_inner[unfolded o_def])
  by simp

lemma disjoint_tensor_id_update_rl[simp]:
  assumes "reference F" and "reference G"
  shows "disjoint (\<lambda>a. F a \<otimes>\<^sub>u id_cblinfun) (\<lambda>a. id_cblinfun \<otimes>\<^sub>u G a)"
  apply (rule disjointI)
  using assms by (auto simp: comp_tensor_op)

lemma disjoint_tensor_id_update_lr[simp]:
  assumes "reference F" and "reference G"
  shows "disjoint (\<lambda>a. id_cblinfun \<otimes>\<^sub>u F a) (\<lambda>a. G a \<otimes>\<^sub>u id_cblinfun)"
  apply (rule disjointI)
  using assms by (auto simp: comp_tensor_op)
...
proof (rule tensor_extensionality)
  ...
  show \<open>(F \<circ> G;F \<circ> H) (a \<otimes>\<^sub>u b) = (F \<circ> (G;H)) (a \<otimes>\<^sub>u b)\<close>
    by (auto simp: reference_pair_apply reference_mult comp_tensor_op)
qed
...
lemma swap_references_right:
  ...
  by (metis assms cblinfun_compose_assoc disjoint_def)

lemmas disjoint_ac_rules = swap_references cblinfun_compose_assoc[symmetric] swap_references_right
...
lemma swap_apply[simp]: "swap (a \<otimes>\<^sub>u b) = (b \<otimes>\<^sub>u a)"
  unfolding swap_def
  by (simp add: Axioms_Quantum.reference_pair_apply Fst_def Snd_def comp_tensor_op) 
...
lemma pair_Fst_Snd: \<open>(Fst; Snd) = id\<close>
  apply (rule tensor_extensionality)
  by (simp_all add: reference_pair_apply Fst_def Snd_def comp_tensor_op)
...
lemma reference_Fst_reference_Snd[simp]:
  ...
  apply (rule tensor_extensionality)
  using assms by (auto simp: reference_pair_apply Fst_def Snd_def reference_mult comp_tensor_op)
...
proof (rule disjointI)
  ...
  proof -
    ...
    also have \<open>\<dots> = H h *\<^sub>u (F; G) (f \<otimes>\<^sub>u g)\<close>
      using \<open>disjoint F G\<close> by (subst reference_pair_apply, auto simp: cblinfun_compose_assoc)
    finally show ?thesis
      by -
  qed
...
proof -
  ...
  have \<open>\<Phi> c d (a \<otimes>\<^sub>u b) = (F \<otimes>\<^sub>r G) (a \<otimes>\<^sub>u b) *\<^sub>u (F' \<otimes>\<^sub>r G') (c \<otimes>\<^sub>u d)\<close> for a b c d
    unfolding \<Phi>_def by (auto simp: reference_pair_apply comp_tensor_op)
  ...
  have \<open>\<Phi> c d (a \<otimes>\<^sub>u b) = (F' \<otimes>\<^sub>r G') (c \<otimes>\<^sub>u d) *\<^sub>u (F \<otimes>\<^sub>r G) (a \<otimes>\<^sub>u b)\<close> for a b c d
    ...
    by (metis assms(1) assms(2) disjoint_def comp_tensor_op)
...
lemma assoc_apply[simp]: \<open>assoc ((a \<otimes>\<^sub>u b) \<otimes>\<^sub>u c) = (a \<otimes>\<^sub>u (b \<otimes>\<^sub>u c))\<close>
  by (auto simp: assoc_def reference_pair_apply Fst_def Snd_def comp_tensor_op)
...
lemma assoc'_apply[simp]: \<open>assoc' (a \<otimes>\<^sub>u (b \<otimes>\<^sub>u c)) =  ((a \<otimes>\<^sub>u b) \<otimes>\<^sub>u c)\<close>
  by (auto simp: assoc'_def reference_pair_apply Fst_def Snd_def comp_tensor_op)
...
proof (rule tensor_extensionality3')
  ...
  show \<open>((F; (G; H)) \<circ> assoc) ((f \<otimes>\<^sub>u g) \<otimes>\<^sub>u h) = ((F; G); H) ((f \<otimes>\<^sub>u g) \<otimes>\<^sub>u h)\<close> for f g h
    by (simp add: reference_pair_apply assoc_apply cblinfun_compose_assoc)
qed
...
proof (rule tensor_extensionality3)
  ...
  show \<open>reference (F; (G; H))\<close>
    by simp
  show \<open>(((F; G); H) \<circ> assoc') (f \<otimes>\<^sub>u g \<otimes>\<^sub>u h) = (F; (G; H)) (f \<otimes>\<^sub>u g \<otimes>\<^sub>u h)\<close> for f g h
    by (simp add: reference_pair_apply assoc'_apply cblinfun_compose_assoc)
qed
