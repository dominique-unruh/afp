...
section \<open>Generic laws about registers, instantiated quantumly\<close>
...
proof (rule compatibleI)
  ...
  proof -
    ...
    also have \<open>\<dots> = H h *\<^sub>u (F; G) (f \<otimes>\<^sub>u g)\<close>
      using \<open>compatible F G\<close> by (subst register_pair_apply, auto simp: cblinfun_compose_assoc)
    finally show ?thesis
      by -
  qed
