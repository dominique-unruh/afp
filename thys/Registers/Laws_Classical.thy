...
section \<open>Generic laws about references, instantiated classically\<close>
...
lemma prereference_tensor_left[simp]: \<open>prereference (\<lambda>b::'b::type update. tensor_update a b)\<close>
  for a :: \<open>'a::type update\<close>
proof -
  have \<open>prereference ((\<lambda>b1::('a\<times>'b) update. (a \<otimes>\<^sub>u Some) *\<^sub>u b1) o (\<lambda>b. tensor_update Some b))\<close>
    by (rule comp_prereference; simp)
  then show ?thesis
    by (simp add: o_def tensor_update_mult)
qed

lemma prereference_tensor_right[simp]: \<open>prereference (\<lambda>a::'a::type update. tensor_update a b)\<close>  
  for b :: \<open>'b::type update\<close>
proof -
  have \<open>prereference ((\<lambda>a1::('a\<times>'b) update. (Some \<otimes>\<^sub>u b) *\<^sub>u a1) o (\<lambda>a. tensor_update a Some))\<close>
...
lemma id_update_tensor_reference[simp]:
  ...
  shows \<open>reference (\<lambda>a::'a::type update. Some \<otimes>\<^sub>u F a)\<close>
  using assms apply (rule reference_comp[unfolded o_def])
  by simp

lemma reference_tensor_id_update[simp]:
  assumes \<open>reference F\<close>
  shows \<open>reference (\<lambda>a::'a::type update. F a \<otimes>\<^sub>u Some)\<close>
  using assms apply (rule reference_comp[unfolded o_def])
  by simp
...
definition reference_tensor  (infixr "\<otimes>\<^sub>r" 70) where
  "reference_tensor F G = reference_pair (\<lambda>a. tensor_update (F a) Some) (\<lambda>b. tensor_update Some (G b))"
...
lemma disjoint_tensor_id_update_left[simp]:
  ...
  assumes "disjoint F G"
  shows "disjoint (\<lambda>a. Some \<otimes>\<^sub>u F a) (\<lambda>a. Some \<otimes>\<^sub>u G a)"
...
lemma disjoint_tensor_id_update_right[simp]:
  fixes F :: "'a::type update \<Rightarrow> 'c::type update" and G :: "'b::type update \<Rightarrow> 'c::type update"
  assumes "disjoint F G"
  shows "disjoint (\<lambda>a. F a \<otimes>\<^sub>u Some) (\<lambda>a. G a \<otimes>\<^sub>u Some)"
  using assms apply (rule disjoint_comp_inner[unfolded o_def])
  by simp

lemma disjoint_tensor_id_update_rl[simp]:
  assumes "reference F" and "reference G"
  shows "disjoint (\<lambda>a. F a \<otimes>\<^sub>u Some) (\<lambda>a. Some \<otimes>\<^sub>u G a)"
  apply (rule disjointI)
  using assms by (auto simp: tensor_update_mult)

lemma disjoint_tensor_id_update_lr[simp]:
  assumes "reference F" and "reference G"
  shows "disjoint (\<lambda>a. Some \<otimes>\<^sub>u F a) (\<lambda>a. G a \<otimes>\<^sub>u Some)"
  apply (rule disjointI)
  using assms by (auto simp: tensor_update_mult)
...
lemma swap_apply[simp]: "swap (a \<otimes>\<^sub>u b) = (b \<otimes>\<^sub>u a)"
  unfolding swap_def
  by (simp add: Axioms_Classical.reference_pair_apply Fst_def Snd_def tensor_update_mult) 
